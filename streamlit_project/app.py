"""
Web App Streamlit — Klasifikasi Suara Lingkungan (ESC-50)
Deployment model hasil notebook ESC50_Klasifikasi_Suara_Lingkungan.ipynb

Menjalankan:
    pip install -r requirements.txt
    streamlit run app.py

Struktur folder yang dibutuhkan (dihasilkan oleh notebook, folder deploy/):
    deploy/config.json
    deploy/model_transfer.keras   (jika best_model = "transfer")
    deploy/model_cnn.keras        (jika best_model = "cnn")
"""
...
# Anchor semua path ke lokasi file app.py ini (bukan working directory),
# agar tetap jalan meski di-deploy dari subfolder repo (mis. Streamlit Cloud).
...
@st.cache_resource
def load_all():
    with open(p("deploy", "config.json")) as f:
        cfg = json.load(f)
    bundle = {"cfg": cfg}
    if cfg["best_model"] == "transfer":
        import tensorflow_hub as hub
        bundle["yamnet"] = hub.load("https://tfhub.dev/google/yamnet/1")
        bundle["head"] = tf.keras.models.load_model(p("deploy", "model_transfer.keras"))
    else:
        bundle["cnn"] = tf.keras.models.load_model(p("deploy", "model_cnn.keras"))
    return bundle
...
st.caption(
    f"Model aktif: {cfg['best_model'].upper()} | "
...
if up:
    ...
    with c2:
        ...
        img = librosa.display.specshow(
            lm, sr=cfg["sr"], hop_length=cfg["hop"], x_axis="time", y_axis="mel", ax=ax2
        )
